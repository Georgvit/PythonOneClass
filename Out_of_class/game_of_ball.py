...
# Подключаем подуле время и рандом
import time
import random

# Создаем окно с игровым полем
tk = Tk()

# Добавляем заголовок
tk.title('Game of ball')

# Фиксируем размер окна
tk.resizable(0, 0)

# Выводим окно по верх остальных окон
tk.wm_attributes('-topmost', 1)

# Создаем холст 400 на 500 пикселей
canvas = Canvas(tk, width=500, height=400, highlightthickness=0)

# Сообщаем холсту что у каждого видимого объекта будут свои координаты
canvas.pack()

# Обновляем окно с холстом
tk.update()

# Создаем класс Ball отвечающий за шарик
class Ball:
    # Создаем конструктор класса
    def __init__(self, canvas, paddle, score, color):
        # Задаем параметры объекта, которые нам передаются в момент создания
        self.canvas = canvas
        self.paddle = paddle
        self.score = score
        ...
        # Создаем список возможных направлений для старта
        start = [-2, -1, 1, 2]

        # Случайное положение на старте
    ...
    def hit_paddle(self, pos):
        ...
        if pos[2] >= paddle_pos[0] and pos[0] <= paddle_pos[2]:
            if pos[3] >= paddle_pos[0] and pos[3] <= paddle_pos[3]:
